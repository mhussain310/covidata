...
class RankingsListView extends View {
  ...
  _generateMarkup() {
    ...
    return `
      ${this._data.slicedResults.map(this._generateMarkupList.bind(this)).join('')}
    `;
  }
  ...
  _generateMarkupList(country, rank) {
    return `
      <li class="rankings__item">
        ...
        <span class="rankings__info">${this._numFmt.format(country.cases)}</span>
        <span class="rankings__info">${this._numFmt.format(country.deaths)}</span>
        <span class="rankings__info">${this._numFmt.format(country.recovered)}</span>
      </li>
    `;
  }
